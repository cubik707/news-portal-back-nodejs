...
describe('RemoveRoleUseCase', () => {
  ...
  let userRepository: { findById: jest.Mock; removeRole: jest.Mock };
  ...
  it('should remove a role from an existing user', async () => {
    const user = makeUser();
    ...
    userRepository.removeRole.mockResolvedValue(updatedUser);

    const result = await useCase.execute('user-id', UserRole.EDITOR);

    expect(result).toBe(updatedUser);
    expect(userRepository.removeRole).toHaveBeenCalledWith('user-id', UserRole.EDITOR);
  });
  ...
  it('should throw UserNotFoundException when user does not exist', async () => {
    ...
    expect(userRepository.removeRole).not.toHaveBeenCalled();
  });
});
