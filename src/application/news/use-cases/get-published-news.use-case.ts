...
import { NewsPublishedFilterDto } from '../dtos/news-published-filter.dto';
...
export class GetPublishedNewsUseCase {
  ...
  async execute(
    query: NewsPublishedFilterDto,
    userId?: string,
  ): Promise<
    { news: News; commentCount: number; likeCount: number; isLikedByCurrentUser: boolean }[]
  > {
    const newsList = await this.newsRepository.findPublishedWithFilters(query);
